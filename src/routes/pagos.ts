...
const connection = require("../conection"); // Notice spelling is conection in this project!
...
router.get("/", (req, res) => {
  ...
  connection.query(queryCajeros, (err, cajeros) => {
    if (err) return res.status(500).json({ error: err.message });
    
    connection.query(queryVentas, [mes, anio], (err, ventas) => {
      if (err) return res.status(500).json({ error: err.message });

      connection.query(queryPagos, [mes, anio], (err, pagos) => {
        if (err) return res.status(500).json({ error: err.message });

        // Build the payroll combining the data
...
router.post("/", (req, res) => {
  ...
  connection.query(checkQuery, [cajero_id, mes, anio], (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
